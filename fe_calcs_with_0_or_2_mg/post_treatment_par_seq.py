...
cond_list = ['5-55NaCl_bb/1-40winCmplx_30winLig']
rep_list = ['1-rep1', '2-rep2', '3-rep3']

with open('par_bb.txt', 'w') as f:
    sys.stdout = f
    for cmpnd in cmpnd_list:
        for cond in cond_list:
            for rep in rep_list:
                dir_str = os.path.join(mainDir,cmpnd,cond,rep,'2-sim_run/BFEE')
                filePathes = [os.path.join(dir_str,'test_bkw_frw_001_MoleculeBound/output/fep_forward.fepout'),
                  os.path.join(dir_str,'001_MoleculeBound/output/fep_backward.fepout'),
                  os.path.join(dir_str,'test_bkw_frw_002_RestraintBound/002.2_ti_forward.log'),
                ...
                lis_str = read_colvar(os.path.join(dir_str,'test_bkw_frw_001_MoleculeBound/colvars.in'))
